package Selenium;/*Тебе предстоит написать несколько автотестов для учебного приложения Яндекс.Метро. Нужно проверить:
...
public class MetroHomePage {
    ...
    // конструктор класса Selenium.MetroHomePage с нужным параметром
    ...
    public void waitForLoadHomePage() {
        // подожди 15 секунд, пока появится веб-элемент с нужным текстом
        new WebDriverWait(driver, Duration.ofSeconds(15)) // WebDriverWait(driver, 8)
    ...
    public void waitForLoadRoute() {
        // подожди 20 секунд, чтобы элемент с нужным текстом стал видимым
    ...
    public void waitForStationVisibility(String stationName) {
        // ждем видимости элемента с нужным текстом из параметра в течение 20 секунд
        new WebDriverWait(driver, Duration.ofSeconds(20))
